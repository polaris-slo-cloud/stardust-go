...
type DeploymentOrchestrator struct {
	...
	specifications []types.IDeploymentSpecification
...
func NewDeploymentOrchestrator() *DeploymentOrchestrator {
	...
	resolver, _ := NewDeploymentOrchestratorResolver([]types.IDeploymentOrchestrator{})
...
func (d *DeploymentOrchestrator) CheckRescheduleAsync(deployment types.IDeploymentSpecification) error {
	...
	for _, spec := range d.specifications {
		wg.Add(1)
		go func(spec types.IDeploymentSpecification) {
			defer wg.Done()
...
func (d *DeploymentOrchestrator) CreateDeploymentAsync(deployment types.IDeploymentSpecification) error {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *DeploymentOrchestrator) DeleteDeploymentAsync(deployment types.IDeploymentSpecification) error {
	d.mu.Lock()
	defer d.mu.Unlock()

	// Remove the deployment specification
	var newSpecifications []types.IDeploymentSpecification
	for _, spec := range d.specifications {
		if spec != deployment {
